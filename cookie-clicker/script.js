    $(document).ready(function () {
        ...
        if (localStorage.cookiecount) {
            ...
            var cookiecount = Number(localStorage.cookiecount);
        } else {
            // variabel neu erstellt
            var cookiecount = 122020;
        }
        ...
        for (i = 0; i < amountBauern; i++) {
            $('.bauernKarte').append('<img class="bauer" src="img/farmer.png">');
        }
        ...
        for (i = 0; i < countFabrik; i++) {
            $('.fabrikKarte').append('<img class="fabrik" src="img/factory.png">');
        }
        ...
        for (i = 0; i < countPortal; i++) {
            $('.portalKarte').append('<img class="portal" src="img/portal.png">');
        }
        // Restliche Variabeln definieren
        if (localStorage.masterCookieCount) {
            var masterCookieCount = Number(localStorage.masterCookieCount);
        } else {
            var masterCookieCount = 0;
        }
        ...
        if (amountBauern >= 3 && cookiecount > 1000 && amountReload > 3) {
            $(".bauernKarte").empty();
            for (i = 0; i < amountBauern; i++) {
                $('.bauernKarte').append('<img class="bauer cat" src="img/nekogirl.png">');
            }
            $('.cookie').attr('onclick', 'playAudio("audio/nyan.mp3")');
        } else {
        ...
        function saveCurrentStatus() {
            ...
            localStorage.setItem('masterCookieCount', masterCookieCount);
        ...
        $('.clickUpgradeWrapper .upgradeNode').click(function () {
            if (cookiecount >= upgradeClickCountCost) { // wen man genug Cookies besitzt
            ...
            } else { // wen man zu wenig Cookies
                var cookieDiffrence = upgradeClickCountCost - cookiecount;
                // Konsole gibt meldung ("Du hast nicht genügend Cookies: dir fehlen " + cookieDiffrence + " Cookies")
                var hours = (new Date()).getHours();
                var minutes = (new Date()).getMinutes();
                var seconds = (new Date()).getSeconds();
                $('.console').append('<p><span>' + hours + ':' + minutes + ':' + seconds + ' ></span> Du hast nicht genügend Cookies: dir fehlen ' + cookieDiffrence + ' Cookies um ein Click-Upgrade zu kaufen.</p>')
            }
        })

        // zweites Upgrade
        // Cookie pro Sekunde + 1
        $('.spanBauernValue').text(amountBauern);
        $('.bauerUpgradeWrapper .upgradeNode').click(function () {
            if (cookiecount >= upgradeBauerCost) { // wen man genug Cookies besitzt
                amountBauern = amountBauern + 1;
                cookiecount = cookiecount - upgradeBauerCost; // Kosten berechnen

                upgradeBauerCost = upgradeBauerCost * 1.5; // kosten erhöhen
                ...
                $('.bauernKarte').append('<img class="bauer" src="img/farmer.png">'); // bauer erstellen in der Karte

            } else { // wen man zu wenig Cookies
                var cookieDiffrence = upgradeBauerCost - cookiecount;
                //alert("Du hast nicht genügend Cookies: dir fehlen " + cookieDiffrence + " Cookies")
                var hours = (new Date()).getHours();
                var minutes = (new Date()).getMinutes();
                var seconds = (new Date()).getSeconds();
                $('.console').append('<p><span>' + hours + ':' + minutes + ':' + seconds + ' ></span> Du hast nicht genügend Cookies: dir fehlen ' + cookieDiffrence + ' Cookies um einen Bauer zu kaufen.</p>')
            }
        })

        setInterval(function () {
            if (amountBauern > 0) {
                cookiecount = cookiecount + amountBauern;
            } else {
        ...
        // Drittes Upgrade
        // Cookie pro Sekunde + 5
        $('.spanFabrikValue').text(amountFabrik);
        $('.fabrikUpgradeWrapper .upgradeNode').click(function () {
            if (cookiecount >= upgradeFabrikCost) { // wen man genug Cookies besitzt
                amountFabrik = amountFabrik + 5;
                countFabrik = countFabrik + 1;
                cookiecount = cookiecount - upgradeFabrikCost; // Kosten berechnen

                upgradeFabrikCost = upgradeFabrikCost * 1.5; // kosten erhöhen
                ...
                $('.fabrikKarte').append('<img class="fabrik" src="img/factory.png">') // Fabrik erstellen in der Karte
            } else { // wen man zu wenig Cookies
                var cookieDiffrence = upgradeFabrikCost - cookiecount;
                //alert("Du hast nicht genügend Cookies: dir fehlen " + cookieDiffrence + " Cookies")
                var hours = (new Date()).getHours();
                var minutes = (new Date()).getMinutes();
                var seconds = (new Date()).getSeconds();
                $('.console').append('<p><span>' + hours + ':' + minutes + ':' + seconds + ' ></span> Du hast nicht genügend Cookies: dir fehlen ' + cookieDiffrence + ' Cookies um eine Fabrik zu kaufen.</p>')
            }
        })

        setInterval(function () {
            if (amountFabrik > 0) {
                cookiecount = cookiecount + amountFabrik;
            } else {
        ...
        // viertes Upgrade
        // Cookie pro Sekunde + 20
        $('.spanPortalValue').text(amountPortal)
        $('.portalUpgradeWrapper .upgradeNode').click(function () {
            if (cookiecount >= upgradePortalCost) { // wen man genug Cookies besitzt
                amountPortal = amountPortal + 20;
                countPortal = countPortal + 1;
                cookiecount = cookiecount - upgradePortalCost; // Kosten berechnen

                upgradePortalCost = upgradePortalCost * 1.5; // kosten erhöhen
                ...
                $('.portalKarte').append('<img class="portal" src="img/portal.png">') // Portal erstellen in der Karte
            } else { // wen man zu wenig Cookies
                var cookieDiffrence = upgradePortalCost - cookiecount;
            //    alert("Du hast nicht genügend Cookies: dir fehlen " + cookieDiffrence + " Cookies")
                var hours = (new Date()).getHours();
                var minutes = (new Date()).getMinutes();
                var seconds = (new Date()).getSeconds();
                $('.console').append('<p><span>' + hours + ':' + minutes + ':' + seconds + ' ></span> Du hast nicht genügend Cookies: dir fehlen ' + cookieDiffrence + ' Cookies um ein Portal zu kaufen.</p>')
            }
        })

        setInterval(function () {
            if (amountPortal > 0) {
                cookiecount = cookiecount + amountPortal;
            } else {
